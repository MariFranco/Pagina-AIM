...
function Microphone (_fft) {
    ...
    this.spectrum = new Uint8Array(FFT_SIZE);
    this.data = [];
    ...
    function startMic (context) {
      ...
      function processSound (stream) {
        this.strings = {
          e5: {
            offset: Math.round(context.sampleRate / 659,3),
            difference: 0
          },
          a4: {
            offset: Math.round(context.sampleRate / 440),
            difference: 0
          },
          d4: {
            offset: Math.round(context.sampleRate / 293,7),
            difference: 0
          },
          g5: {
            offset: Math.round(context.sampleRate / 195),
            difference: 0
          }
        }
    ...
    this.mapRawSound = function(_me, _total, _min, _max){
      ...
      if (self.spectrum.length > 0) {
        ...
        //console.log(self.spectrum[new_freq]);
        // map the volumes to a useful number
        //console.log(map(self.spectrum[new_freq], 0, self.peak_volume, min, max));
